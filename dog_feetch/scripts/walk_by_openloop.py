...
"""
This script runs the educational open-loop gait and inverse-kinematics controller.
This script runs the educational open-loop gait and inverse-kinematics controller.
This script runs the educational open-loop gait and inverse-kinematics controller.
"""
...
# This script runs the educational open-loop gait and inverse-kinematics controller.
default_offset = {
    GaitType.TROT: [0, 0.5, 0.5, 0],              #  180
    GaitType.CRAWL: [0, 1 / 4, 2 / 4, 3 / 4],
}

# This script runs the educational open-loop gait and inverse-kinematics controller.
default_stand_frac = {
    GaitType.TROT: 3 / 4,                         # 75% 25%
    GaitType.CRAWL: 3 / 4,
}
...
class BodyState(TypedDict):
    ...
    xm: float; ym: float; zm: float        # X, Y, Z
...
def setup_gui_sliders():
    ...
    sliders = {
        ...
        "yaw": p.addUserDebugParameter("yaw", -1, 1, 0),                   #  Yaw
        "pitch": p.addUserDebugParameter("pitch", -1, 1, 0),               #  Pitch
        "roll": p.addUserDebugParameter("roll", -1, 1, 0),                 #  Roll
...
def main():
    ...
    parser.add_argument("--dt", type=float, default=0.02) # 0.02 ( 50Hz )
    ...
    # This script runs the educational open-loop gait and inverse-kinematics controller.
    gait = GaitController(standby)   # 1)
    ik = Kinematics()                 # 2)
    hwi = HWI(usb_port=args.usb_port) # 3)

    # This script runs the educational open-loop gait and inverse-kinematics controller.
    hwi.turn_on()
    pb, sliders = setup_gui_sliders()
